...
PIPELINE_FILE = 'preprocessor_pipeline.joblib' # We will save the preprocessor
...
model_df['emp_length'] = model_df['emp_length'].str.replace(r'< 1 year', '0 years').str.replace(r'10\+ years', '10 years').str.replace(r' years?', '', regex=True).astype(float)

# --- NEW: Define all our features ---
numeric_features = [
    'loan_amnt', 
    'int_rate', 
    'annual_inc', 
...
# --- 3. NEW: Create a Preprocessing Pipeline ---
...
# --- 5. Train XGBoost Default Model (on Transformed Data) ---
...
# We use .named_steps to pass eval_set to the 'classifier' step
...
# We also save just the preprocessor, as the env will need it
joblib.dump(preprocessor, PIPELINE_FILE)
joblib.dump(all_features, 'model_features.joblib') # Save new feature list

print(f"\nModel pipeline saved as '{MODEL_FILE}'")
print(f"Preprocessor saved as '{PIPELINE_FILE}'")
