...
try:
    ...
    # Fallback to sklearn if XGBoost fails
...
# Update data loading path and add permission handling
candidates = [Path('data') / 'accepted_2007_to_2018Q4.csv', Path('accepted_2007_to_2018Q4.csv')]
for p in candidates:
    if p.exists():
        print(f'Attempting to load data from: {p}')
        try:
            df = pd.read_csv(p, low_memory=False, parse_dates=['issue_d'])
            print(f"Data loaded successfully. Shape: {df.shape}")
            break
        except PermissionError as pe:
            print(f'PermissionError reading {p}: {pe}')
            # Try copying to temp
            try:
                tmp_dir = Path(tempfile.gettempdir())
                tmp_file = tmp_dir / p.name
                shutil.copy2(p, tmp_file)
                df = pd.read_csv(tmp_file, low_memory=False, parse_dates=['issue_d'])
                print(f"Data loaded from temp. Shape: {df.shape}")
                break
            except Exception as e_copy:
                print('Failed to load data:', e_copy)
                df = None
                break
...
if df is None:
    raise ValueError("Could not load dataset. Please ensure the file is in data/ or current directory.")
...
# Switch to XGBoost for GPU support
print("\nTraining the Default Model with XGBoost (GPU enabled if available)...")
if use_gpu:
    default_model = XGBClassifier(tree_method='gpu_hist', n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42, n_iter_no_change=5, validation_fraction=0.1) # type: ignore
else:
    default_model = XGBClassifier(tree_method='hist', n_estimators=100, learning_rate=0.1, max_depth=3, random_state=42, n_iter_no_change=5, validation_fraction=0.1) # type: ignore
